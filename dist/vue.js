...
(function (global, factory) {
   ...
   * 是否为标准对象，一般是通过{}或new Object创造出来的对象
   */
   ...
   * is in that map.
   * 构建一个map对象，并且返回一个可以检查指定key是否在该map中的方法，下面一系列方法都会用到
  ...
  function makeMap(
    ...
    var map = Object.create(null);  // 将其__proto__设置为null
    var list = str.split(',');  // 分割字符串
    for (var i = 0; i < list.length; i++) {
      map[list[i]] = true;
    }  // 输入str为'li,sda,asd’，得到map类似：{ li: true, sda: true, asd: true }
    return expectsLowerCase  // 根据传入的第二个参数，决定是否将键小写
   ...
   * splice(index, count) 从index位置开始删除count个元素，返回新数组
   */
